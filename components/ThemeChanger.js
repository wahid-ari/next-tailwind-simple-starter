  ...
  if (simple) {
    return theme == 'dark' ? (
      <button
        ...
        aria-label='Light'
        className='focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-500 rounded p-0.5'
      >
        <SunIcon className='h-6 w-6 dark:text-neutral-300 dark:hover:text-neutral-100 transition-all' />
      </button>
    ) : (
      <button
    ...
    <>
      ...
      >
        ...
        <span aria-hidden={true}>
          <SunIcon className={`${theme === 'dark' ? 'text-white bg-white' : ''}h-5 w-5`} />
        </span>
        <span aria-hidden={true}>
